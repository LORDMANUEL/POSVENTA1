...
def _slug(value: str) -> str:
    ...
    if len(normalized) < 2:
        raise HTTPException(status_code=422, detail="Slug de tienda inválido")
    return normalized[:80]


class BootstrapIn(BaseModel):
    store_name: str = Field(min_length=2, max_length=160)
    store_slug: str | None = Field(default=None, min_length=2, max_length=80)
...
def _cash_session_for_user(
    ...
    *,
    ...
    if user.role == UserRole.CASHIER and session.user_id != user.id:
        raise HTTPException(
            status_code=403,
            detail="No puede consultar la caja de otro usuario",
        )
    return session


def _current_cash_for_user(
    db: Session,
    user: User,
    *,
    for_update: bool = False,
...
def bootstrap(
    ...
    bootstrap_token: Annotated[
        str | None,
        Header(alias="X-Bootstrap-Token"),
    ] = None,
    ...
    if not settings.bootstrap_token:
        raise HTTPException(
            status_code=503,
            detail="Bootstrap deshabilitado: configure MZ_BOOTSTRAP_TOKEN",
        )
    if not bootstrap_token or not secrets.compare_digest(
        bootstrap_token,
        settings.bootstrap_token,
    ):
        raise HTTPException(status_code=403, detail="Código de instalación inválido")

    tenant_slug = _slug(payload.store_slug or payload.store_name)
    tenant = Tenant(name=payload.store_name, slug=tenant_slug)
    db.add(tenant)
    try:
        db.flush()
        branch = Branch(
        ...
        raise HTTPException(
            status_code=409,
            detail="La instalación inicial entró en conflicto; revise slug/correo y reintente",
        ) from exc
    db.refresh(owner)
...
def login(
    ...
    email = username
    if ":" in username:
        tenant_slug, email = username.split(":", 1)
    ...
    if tenant_slug:
        ...
        if len(matches) > 1:
            raise HTTPException(
                status_code=409,
                detail="Ese correo existe en varias tiendas; ingrese tenant:correo",
            )
    ...
    return [
        {
            ...
            "quantity": quantity_text(balance.quantity),
        }
...
def move_inventory(
    ...
    AuditService.record(
        ...
        payload.product_id,
        {
            "delta": quantity_text(payload.quantity_delta),
...
def open_cash(
    ...
    active = _current_cash_for_user(db, user, for_update=True)
    if active:
        raise HTTPException(
            status_code=409,
            detail="Ya existe una caja abierta para este usuario",
        )
    ...
    AuditService.record(
        ...
        "cash_session",
        session.id,
        {"opening": str(payload.opening_amount), "branch_id": branch_id},
    )
    db.commit()
    db.refresh(session)
...
def cash_summary(
    ...
    return {
        ...
        "closing_amount": (
            str(session.closing_amount)
            if session.closing_amount is not None
            else None
        ),
        "difference": (
            str(money(Decimal(session.closing_amount) - expected))
            if session.closing_amount is not None
            else None
        ),
...
def create_cash_movement(
    ...
    session = _cash_session_for_user(
        db,
        session_id,
        user,
        for_update=True,
    ...
    signed_amount = money(
        payload.amount
        if payload.movement_type == "cash_in"
        else -payload.amount
    )
...
def close_cash(
    ...
    AuditService.record(
        ...
        session.id,
        {
            ...
            "counted": str(session.closing_amount),
...
def create_print_job(
    ...
    AuditService.record(
        ...
        "print_job",
        job.id,
        {"type": job.job_type, "branch_id": branch_id},
    )
    db.commit()
