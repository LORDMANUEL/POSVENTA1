...
const roleViews = {
  ...
  sales: ['pos', 'products', 'customers', 'deliveries'],
...
function App() {
  ...
  const fullViews = ['home', 'pos', 'products', 'inventory', 'cash', 'customers', 'suppliers', 'purchases', 'transfers', 'deliveries', 'admin'];
  ...
  const nav = allowed.filter((item) => !(item === 'admin' && !['owner', 'admin'].includes(me.role)));
  return <div className="app-shell"><aside className="sidebar"><div className="logo"><span>MZ</span><div><strong>Mily Zebra</strong><small>Commerce OS</small></div></div><nav>{nav.map((item) => <button key={item} className={view === item ? 'active' : ''} onClick={() => setView(item)}>{labels[item]}</button>)}</nav><div className="user-card"><strong>{me.full_name}</strong><small>{me.role}</small><button className="link" onClick={() => { api.setToken(''); location.reload(); }}>Cerrar sesión</button></div></aside><main className="workspace"><header><div><p className="eyebrow">Roatán · operación en vivo</p><h1>{labels[view] || 'Mily Zebra'}</h1></div><div className="status"><i /> API conectada</div></header>{view === 'home' && <div className="dashboard"><article><small>Productos</small><strong>{products.length}</strong><span>catálogo activo</span></article><article><small>Unidades visibles</small><strong>{inventory.reduce((sum, row) => sum + Number(row.quantity), 0)}</strong><span>en inventario</span></article><article><small>Modo</small><strong>{queryMode || 'completo'}</strong><span>{me.role}</span></article></div>}{view === 'pos' && <Pos products={products} refresh={refresh} />}{view === 'products' && <section className="panel"><div className="panel-title"><div><p className="eyebrow">Catálogo</p><h2>Nuevo producto</h2></div></div><ProductForm onSaved={refresh} /><div className="product-list">{products.map((p) => <div key={p.id}><span><strong>{p.name}</strong><small>{p.sku} · {p.category}</small></span><b>{money.format(Number(p.sale_price))}</b></div>)}</div></section>}{view === 'inventory' && <Inventory rows={inventory} products={products} refresh={refresh} />}{view === 'cash' && <Cash />}{view === 'customers' && <CustomersView />}{view === 'suppliers' && <SuppliersView />}{view === 'purchases' && <PurchasesView products={products} refreshInventory={refresh} />}{view === 'transfers' && <TransfersView products={products} refreshInventory={refresh} />}{view === 'deliveries' && <DeliveriesView me={me} />}{view === 'admin' && <UsersDevicesView />}</main></div>;
}
