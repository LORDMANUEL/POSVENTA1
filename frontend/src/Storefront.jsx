...
function ProductImage({ product, tone = 0, compact = false }) {
  if (product.primary_image_url) {
    return (
      <div className={`sf-product-image sf-product-image-real ${compact ? 'compact' : ''}`}>
  ...
  return (
    <div className={`sf-product-image tone-${tone % 4} ${compact ? 'compact' : ''}`}>
      {!compact && <span>{product.category}</span>}
    ...
    <div className="sf-overlay" onClick={onClose}>
      <aside className="sf-cart" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="sf-cart-lines">
          {cart.length === 0 && <p className="sf-muted">Tu bolsa está vacía.</p>}
          {cart.map((item) => (
            <div className="sf-cart-line" key={item.id}>
              <ProductImage product={item} compact />
              <div className="sf-cart-copy">
                <strong>{item.name}</strong>
                <small>{item.size || 'Talla única'} · {item.color || 'Color surtido'}</small>
                <div className="sf-qty">
                  <button onClick={() => onQty(item.id, -1)}>−</button><span>{item.qty}</span><button onClick={() => onQty(item.id, 1)}>+</button>
                </div>
              </div>
              <b>{money.format(Number(item.sale_price) * item.qty)}</b>
            </div>
          ))}
        </div>
        <footer>
          <div><span>Total</span><strong>{money.format(total)}</strong></div>
          <button className="sf-primary" disabled={!cart.length} onClick={onCheckout}>Continuar compra</button>
        </footer>
      </aside>
    </div>
  );
...
function Checkout({ cart, onBack, onCompleted }) {
  ...
  const total = cart.reduce((sum, item) => sum + item.qty * Number(item.sale_price), 0);
  ...
  return (
    <main className="sf-checkout">
      <button className="sf-back" onClick={onBack}>← Volver a la tienda</button>
      <div className="sf-checkout-grid">
        <section>
          <p className="sf-kicker">Compra segura</p><h1>Finaliza tu pedido</h1>
          <p>La disponibilidad se vuelve a validar en el servidor y se reserva antes de confirmar el pedido.</p>
          <form onSubmit={submit}>
            <label>Nombre completo<input required value={form.full_name} onChange={(e) => setForm({ ...form, full_name: e.target.value })} /></label>
            <label>WhatsApp / teléfono<input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} /></label>
            ...
            <div className="sf-radio-row">
              <label><input type="radio" checked={form.fulfillment_method === 'pickup'} onChange={() => setForm({ ...form, fulfillment_method: 'pickup' })} /> Recoger en tienda</label>
              <label><input type="radio" checked={form.fulfillment_method === 'delivery'} onChange={() => setForm({ ...form, fulfillment_method: 'delivery' })} /> Entrega</label>
            </div>
            {form.fulfillment_method === 'delivery' && <label>Dirección de entrega<textarea required value={form.delivery_address} onChange={(e) => setForm({ ...form, delivery_address: e.target.value })} /></label>}
            <label>Método de pago<select value={form.payment_method} onChange={(e) => setForm({ ...form, payment_method: e.target.value })}><option value="manual_transfer">Transferencia bancaria</option><option value="cash_on_delivery">Pago contra entrega</option></select></label>
            {error && <div className="sf-error">{error}</div>}
            <button className="sf-primary" disabled={busy}>{busy ? 'Creando pedido…' : 'Confirmar pedido'}</button>
          </form>
        </section>
        <aside className="sf-summary"><p className="sf-kicker">Resumen</p>{cart.map((item) => <div key={item.id}><span>{item.qty} × {item.name}</span><strong>{money.format(item.qty * Number(item.sale_price))}</strong></div>)}<footer><span>Total</span><strong>{money.format(total)}</strong></footer></aside>
      </div>
    </main>
  );
}
